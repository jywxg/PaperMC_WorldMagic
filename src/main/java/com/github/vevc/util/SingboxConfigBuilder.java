...
public class SingboxConfigBuilder {
    ...
    public SingboxConfigBuilder(AppConfig config) {
        this.config = config;
        this.countryCode = CountryCodeUtil.getCountryCode();
    }
    ...
    private JsonObject buildTuicInbound() {
        ...
        certificate.addProperty("certificate", "self_sign");
        tls.add("certificate", certificate);
    ...
    /**
     * Generate node name with country code
     * Format: {CountryCode}-zv-{Protocol}
     * Example: US-zv-hysteria2, JP-zv-vmess-ws
     */
    private String generateNodeName(String protocol) {
        return countryCode + "-zv-" + protocol;
    }
    ...
    /**
     * Generate subscription file names with country code
     * Format: {CountryCode}-zv-{Protocol}
     */
